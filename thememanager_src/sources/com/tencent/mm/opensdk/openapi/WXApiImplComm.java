...
class WXApiImplComm {
    ...
    public static boolean validateAppSignature(Context context, Signature[] signatureArr, boolean z2) {
        String str;
        if (z2) {
            for (Signature signature : signatureArr) {
                String lowerCase = signature.toCharsString().toLowerCase();
                Log.m18252d(TAG, "check signature:" + lowerCase);
    ...
    public static boolean validateAppSignatureForPackage(Context context, String str, boolean z2) {
        if (z2) {
            try {
                return validateAppSignature(context, context.getPackageManager().getPackageInfo(str, 64).signatures, z2);
            } catch (PackageManager.NameNotFoundException unused) {
                return false;
            }
        }
        Log.m18252d(TAG, "ignore wechat app signature validation");
        return true;
    }
}
