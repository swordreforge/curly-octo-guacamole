...
void main() {
    ...
    vec2 texCoord =  refractPos / uTexScaleOffset.xy + 0.5 + uTexScaleOffset.zw;
    texCoord = clamp(texCoord,0.0,1.0);
    fragColor = texture(uTexture, (vTexMatrix * vec4(texCoord, 0, 1)).xy);
...
//    fragColor = texture(uTexture, (vTexMatrix * vec4(vTexCoord, 0, 1)).xy);
}
