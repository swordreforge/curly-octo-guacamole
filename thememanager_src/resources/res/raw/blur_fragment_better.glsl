...
//uniform  float uToRad ;
//uniform  float uFromRad;
//uniform  float uAsp;
uniform  vec2 uDirection;
uniform int uReverse;
uniform vec4 uASPMAPFROMTORAD;
...
const float sigma  = 16.0; // Gaussian sigma
vec4 blur( highp vec2 loc ,float samples,vec2 dir)
{
    vec4 acc = vec4( 0.0 );// accumulator
    float norm = 0.0;
    int support = int(samples);
    support = min(support,50);
    float singma_double = sigma * sigma;
    for (int i = 0; i <= support; i+=2) {
        float radius = float(i);
        float coeff = exp(-0.5 * radius * radius / (singma_double));
        //float coeff = uCoffs[i];
        //float coeff = uCoffs[i/2];
        if(uReverse  == 0)
            //acc += (texture(uTexture, loc + radius * dir) + texture(uTexture, loc - radius * dir)) * coeff;
            acc += (texture(uTexture, (vTexMatrix * vec4(loc.xy, 0, 1)).xy + radius * dir) + texture(uTexture, (vTexMatrix * vec4(loc.xy, 0, 1)).xy - radius * dir)) * coeff;
        else
            acc += (texture(uTexture2, loc + radius * dir) + texture(uTexture2, loc - radius * dir)) * coeff;

        norm += 2.0 * coeff;
    }
    acc *= 1.0/max(norm,1e-7);
    // Output to screen
    return acc;
}
...
//        UV = uv;
    UV = uv;
    vec2 st = UV;
    st.y = UV.y * uASPMAPFROMTORAD.r;
    st = st * 2. - 1.;
    st.y -= 0.15;
    st /= max(uASPMAPFROMTORAD.g, 1e-7);
    st = st * 0.5 + 0.5;

    vec4 noiseColor = texture(uNoise, vec2(st.x, 1.0 - st.y));
    float blurRadius = noiseColor.r;
    blurRadius = blurRadius * (uASPMAPFROMTORAD.a - uASPMAPFROMTORAD.b) + uASPMAPFROMTORAD.b;
    fragColor = blur(UV, blurRadius, uDirection);
